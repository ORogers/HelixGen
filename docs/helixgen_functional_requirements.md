# 🎛 Functional Requirements — `helixgen`

## 1. Overview

**Purpose:**  
`helixgen` is a command-line utility for generating and validating Line 6 **Helix (.hlx)** preset files. It allows structured, programmatic creation of Helix presets from a defined signal chain and ensures schema-compliant output compatible with **HX Edit** and **Helix Native**.
...
If a user requests a model or parameter not present in this dataset, `helixgen` must reject or warn, ensuring only known Helix models can be generated.

---

## 3. Functional Requirements

### FR-1 — Generate Preset (`helixgen generate`)
...
### FR-2 — Validate Preset (`helixgen validate`)
...
### FR-3 — Inspect Preset (`helixgen inspect`)
...
✅ `helixgen validate` correctly detects invalid or missing model IDs.  
