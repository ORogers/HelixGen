...
1. Download `HelixPy-<version>.dmg` from [Releases][releases].
2. Open it and drag **HelixPy** to Applications.
...
The dialog says HelixPy "cannot be opened because it is from an unidentified
...
about the app. Everything HelixPy is built from is in this repository, and the
...
```bash
xattr -dr com.apple.quarantine /Applications/HelixPy.app
```
...
That gives you `hlxgen`. To talk to a pedal you also need the USB stack and
libusb:

```bash
brew install libusb
pipx install 'helixpy[usb]'
```
...
Talking to the pedal needs [HX Edit][hxedit] installed - HelixPy reads Line 6's
own data files out of it, because they are not ours to distribute. See
[legal.md](legal.md).

[hxedit]: https://line6.com/software/

HelixPy finds it in `/Applications/Line6/` or `/Applications/`, and falls back
...
```bash
export HLXGEN_HX_EDIT="/path/to/HX Edit.app"
```
