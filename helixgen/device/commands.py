...
These live apart from ``helixgen.cli`` so that importing the CLI does not drag in
...
from helixgen.device.codec import overlay
...
def run_devices(args: argparse.Namespace) -> int:
    ...
    from helixgen.device.usb import DeviceError, Session, find_devices
    ...
    for info in devices:
        ...
        if not info.verified:
            print("  NOTE: this model has not been tested with helixgen's USB support.")
...
def run_pull(args: argparse.Namespace) -> int:
    ...
    from helixgen.device.usb import DeviceError, Session
...
def run_push(args: argparse.Namespace) -> int:
    ...
    * **document** -- a whole-document ``op 21`` write. Touches the chain as one
    ...
    """
    from helixgen.device.editor import EditError, apply_tone
    from helixgen.device.usb import DeviceError, Session
    from helixgen.device.writer import WriteAbortedError
