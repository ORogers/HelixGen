...
This deliberately does **not** shell out to ``hlxgen`` as a subprocess and
does **not** call ``hlxgen.cli._generate_from_chain`` either - that function
...
(:func:`hlxgen.llm.generate_chain_from_prompt`,
:func:`hlxgen.generator.generate_preset`,
:class:`hlxgen.validator.PresetValidator`) directly, so callers get a
...
described preset's default output path comes from* - ``hlxgen/cli.py``'s
...
from hlxgen.cli import DEFAULT_DATASET, DEFAULT_SCHEMA, DEFAULT_TEMPLATE_PATH
from hlxgen.cli import _default_describe_output as default_describe_output
...
def generate_tone(
    ...
    Raises :class:`~hlxgen.llm.LLMGenerationError` if the LLM round trips
    fail, :class:`GenerationValidationError` if the assembled preset doesn't
    pass the same gate the CLI enforces, or :class:`~hlxgen.dataset.ModelCatalogError`
