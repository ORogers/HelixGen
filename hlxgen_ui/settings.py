...
Everything here is **persisted** to ``hlxgen.config``'s file, so a choice made
once survives a restart. The OpenAI key is not part of this dataclass: it is
read and written through ``hlxgen.llm`` so that the CLI resolves the same key
...
from hlxgen.config import read_settings, write_settings
