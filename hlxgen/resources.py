"""Where the data files live, wherever hlxgen is running from.
...
The files now ship inside :mod:`hlxgen.data`, and everything that needs one asks
...
def _data_file(name: str) -> Path:
    ...
    """
    with as_file(files("hlxgen.data").joinpath(name)) as path:
        return Path(path)
...
def default_output_dir() -> Path:
    ...
    """
    if is_frozen():
        return Path.home() / "Documents" / "HelixPy" / "presets"
