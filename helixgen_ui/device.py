"""The UI's view of the pedal, built on ``helixgen.device`` - the real,
hardware-verified USB implementation that backs ``helixgen devices/pull/push``.
...
* runs the same upload path ``helixgen push`` uses (:func:`apply_tone`).

Every call in this module is safe to run off the UI thread and none of them
touch Qt, so the workers in :mod:`helixgen_ui.workers` can call them directly.
"""
...
logger = logging.getLogger("helixgen_ui.device")
...
def _device_error() -> type[Exception]:
    from helixgen.device.usb import DeviceError
...
def find_device() -> DeviceSummary | None:
    ...
    """
    try:
        from helixgen.device.usb import find_devices
...
def read_slots(
    ...
    *,
    ...
    Uses :meth:`helixgen.device.usb.Session.read_slot`, which reads a slot's
    ...
    try:
        ...
        for slot in range(count):
            ...
            # exactly as `helixgen backup` does.
...
def push_preset(
    ...
    *,
    ...
    This is the same path ``helixgen push`` takes by default: surgical edits via
    :func:`helixgen.device.editor.apply_tone`, which is the only transport that
    can change a block's model. ``apply_tone`` opens and renews its own
    sessions, so no session is held here.

    Returns the report ``helixgen push`` prints, for display in the UI.
    """
    ...
    from helixgen.device.editor import apply_tone
    from helixgen.device.hxedit import find_symbol_table
    ...
    name = preset.get("data", {}).get("meta", {}).get("name") or "helixgen"
...
def chain_from_preset(preset: dict[str, Any], catalog: Any) -> list[ChainBlock]:
    ...
    from helixgen.dataset import ModelCatalogError
